...
[numthreads(32, 1, 1)]
void CS_DamageParticle(int3 _id : SV_DispatchThreadID)
{
    ...
    else
    {
        ...
        float3 pos = ParticleBuffer[_id.x].vPos;
        pos.x = ParticleBuffer[0].vPos.x + 30.f * _id.x; // 시작위치로 부터의 Offset 을 계산 
        pos.y += fSpeed * 0.2f * fDT;
 
        ParticleBuffer[_id.x].vPos = pos;
        ParticleBuffer[_id.x].vColor = vColor;
        
        float3 scale = vScale;
        if(_id.x == 1 || _id.x == 0)
            scale = scale * 1.2f;
        ParticleBuffer[_id.x].vScale = scale;
    }
}
