...
void CCollider2D::SaveToFile(FILE* _pFile)
{
	...
	fwrite(&m_eColliderType, sizeof(COLLIDER2D_TYPE), 1, _pFile);
	...
	wstring key;
	if (nullptr != m_pMesh.Get())
	{
		key = m_pMesh.Get()->GetKey();
		SaveWString(key, _pFile);
	}

	if (nullptr != m_pMtrl.Get())
	{
		key = m_pMtrl.Get()->GetKey();
		SaveWString(key, _pFile);
	}

	fwrite(&m_iCollisionCount, sizeof(int), 1, _pFile);

}

void CCollider2D::LoadFromFile(FILE* _pFile, bool IsPrevRead)
{

	CComponent::LoadFromFile(_pFile);

	fread(&m_eColliderType, sizeof(COLLIDER2D_TYPE), 1, _pFile);
	...
	fread(&m_iCollisionCount, sizeof(int), 1, _pFile);
}
