...
void CAnimator2D::SaveToFile(FILE* _pFile)
{
	...
	int mapAnimCnt = m_mapAnim.size();
	fwrite(&mapAnimCnt, sizeof(int), 1, _pFile);
...
void CAnimator2D::LoadFromFile(FILE* _pFile, bool IsPrevRead)
{
	CComponent::LoadFromFile(_pFile);

	int mapAnimCnt;
	fread(&mapAnimCnt, sizeof(int), 1, _pFile);

	for (int i = 0; i < mapAnimCnt; ++i)
	{
		wstring strName;
	...
	m_pCurAnim = new CAnimation2D;
	m_pCurAnim->LoadFromFile(_pFile);
