#ifndef _PARTICLE_STAR_HYPO_
#define _PARTICLE_STAR_HYPO_
...
void CS_Particle(int3 _id : SV_DispatchThreadID)
{
    ...
    if (0 == ParticleBuffer[_id.x].Alive)
    {
        ...
        if (ParticleBuffer[_id.x].Alive)
        {
            ...
            ParticleBuffer[_id.x].vDir = ParticleBuffer[_id.x].vPos; // 파티클 시작위치 
                        
            ParticleBuffer[_id.x].vPos.x += CreateRange * vRandom.x;
            ParticleBuffer[_id.x].vPos.y += CreateRange * vRandom.y;
            
            if(vRandom.x <= 1.5f)
            {
                ParticleBuffer[_id.x].vPos.x += CreateRange * vRandom.x * -1.f;
                ParticleBuffer[_id.x].vPos.y += CreateRange * vRandom.y;
            }
            

            float a = 140.f, b = 100.f, t = ParticleBuffer[_id.x].m_fCurTime * 10.f;
        
            ParticleBuffer[_id.x].vPos.x += (a - b) * cos(t) + b * cos(((a / b - 1) * t));
            ParticleBuffer[_id.x].vPos.y += (a - b) * sin(t) - b * sin(((a / b - 1) * t));
    ...
    else
    {
        ...
        float a = 70.f, b = 100.f,  t = ParticleBuffer[_id.x].m_fCurTime * 5.f;
        
        
        ParticleBuffer[_id.x].vPos.x = ParticleBuffer[_id.x].vDir + (a - b) * cos(t) + b * cos(((a / b - 1) * t));
        ParticleBuffer[_id.x].vPos.y = ParticleBuffer[_id.x].vDir + (a - b) * sin(t) - b * sin(((a / b - 1) * t));
