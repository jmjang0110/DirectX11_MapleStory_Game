	...
	, m_bRepeat(_origin.m_bRepeat)
{
	...
	//  map<wstring, CAnimation2D*> m_mapAnim; : บนป็ 
...
void CAnimator2D::CreateAnim(const wstring& _strName, Ptr<CTexture> _pAtlas, Vec2 _vLeftTopPixel, Vec2 _vSlicePixel, Vec2 _vStepPixel, float _fDuration, int _iFrameCount)
{
	assert(!FindAnim(_strName));

	CAnimation2D* anim = new CAnimation2D;
	anim->SetName(_strName);
	anim->Create(_pAtlas, _vLeftTopPixel, _vSlicePixel, _vStepPixel, _fDuration, _iFrameCount);
