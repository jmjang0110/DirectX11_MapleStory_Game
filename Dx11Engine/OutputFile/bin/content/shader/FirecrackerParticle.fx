...
void CS_Particle(int3 _id : SV_DispatchThreadID)
{
    ...
    if (0 == ParticleBuffer[_id.x].Alive)
    {
        ...
        if (ParticleBuffer[_id.x].Alive)
        {
            ...
            ParticleBuffer[_id.x].vScale = float3(1.f, 1.f, 1.f);                 // 파티클 초기 크기 
            ...
            if (400 <= _id.x && _id.x < 500)
            {
               
               // ParticleBuffer[_id.x].vPos.y += (ParticleBuffer[_id.x].vScale.y + 50.f);
                ParticleBuffer[_id.x].m_fMaxTime *= 10.f;
                ParticleBuffer[_id.x].vColor = float4(204.f / 255.f, 153.f / 255.f, 204.f / 255.f, 1.f);
                
            }
            else if (500 <= _id.x && _id.x < 600)
            {
               // ParticleBuffer[_id.x].vPos.y += (ParticleBuffer[_id.x].vScale.y + 50.f);
                ParticleBuffer[_id.x].m_fMaxTime *= 10.f;
                ParticleBuffer[_id.x].vColor = float4(204.f / 255.f, 153.f / 255.f, 204.f / 255.f, 1.f);
            ...
            {
                ParticleBuffer[_id.x].vPos.y += 20.f;
                ParticleBuffer[_id.x].vColor = float4(255.f / 255.f, 204.f / 255.f, 255.f / 255.f, 1.f);
    ...
    else
    {
        ...
        ParticleBuffer[_id.x].vScale.y  *= 8.f;
        ParticleBuffer[_id.x].vScale.x  = 4.f;
        
        
      
        if (400 <= _id.x && _id.x < 500)
        {
            ParticleBuffer[_id.x].vScale.y /= 20.f;
            ParticleBuffer[_id.x].vScale.x = ParticleBuffer[_id.x].vScale.y;
            
            ParticleBuffer[_id.x].vPos.y += fDT * fSpeed * 0.1f;
            ...
            ParticleBuffer[_id.x].vColor.rgb -= 0.0001f;
        ...
        else if (500 <= _id.x && _id.x < 600)
        {
            ...
            ParticleBuffer[_id.x].vPos.y += fDT * fSpeed * 0.1f;
            ParticleBuffer[_id.x].vPos.z = sin(ParticleBuffer[_id.x].m_fCurTime * 3.f) * 50.f + WorldPos.z;
            ParticleBuffer[_id.x].vPos.x = -1 * cos(ParticleBuffer[_id.x].m_fCurTime * 3.f) * 50.f + WorldPos.x;
        ...
        {
            ParticleBuffer[_id.x].vScale.y = 2.f;
            ParticleBuffer[_id.x].vColor.rgb -= 0.0005f;
            
            ParticleBuffer[_id.x].vPos.y += fDT * fSpeed * 0.8f;
